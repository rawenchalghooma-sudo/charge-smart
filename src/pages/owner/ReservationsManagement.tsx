...
function getStatusClass(status: ReservationStatus) {
  ...
  if (status === "Rejetée") return "bg-rose-50 text-rose-600";
  if (status === "Annulée") return "bg-slate-100 text-slate-600";
...
function formatDuration(startTime?: string, endTime?: string) {
  ...
  if (!isNaN(start.getTime()) && !isNaN(end.getTime())) {
    ...
    return diff > 0 ? `${diff} min` : "Non renseignée";
  }
...
function parseUrgencyFromQr(qrCodeId?: string | null): number {
...
function mapReservation(apiReservation: ReservationApi): Reservation {
  return {
    ...
    qrCodeId: apiReservation.qrCodeId || null,
...
function ReservationCard({
  ...
  const isPending = reservation.status === "En attente";
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex-1">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            {reservation.status === "Confirmée" && reservation.qrCodeId ? (
...
export default function ReservationsManagement() {
  ...
  const handleReject = async (id: number) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/reservations/owner/${id}/reject`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      console.error(err);
      alert("Impossible de rejeter la réservation.");
    } finally {
      setActionLoadingId(null);
    }
  };
  ...
  const filteredReservations = useMemo(() => {
    return reservations.filter((reservation) => {
      const matchesSearch =
        ...
        reservation.station.toLowerCase().includes(searchTerm.toLowerCase());

      const matchesDate =
        !selectedDate || reservation.date.slice(0, 10) === selectedDate;
  ...
  const todayCount = reservations.filter(
    (reservation) =>
      reservation.date.slice(0, 10) === new Date().toISOString().slice(0, 10)
  ).length;
